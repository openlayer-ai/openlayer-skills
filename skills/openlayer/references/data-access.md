...
## Tier 1 — MCP (preferred when connected)

If the Openlayer MCP server is available, use its tools — they are typed and agent-friendly. Domain
tools cover the common reads/writes; `call_openlayer_api(method, endpoint, query_params, body)` is the
escape hatch for anything uncovered. See `references/mcp-setup.md` for the full tool catalog.

## Tier 2 — SDK (typed client)
...
Last resort when neither MCP nor SDK is set up. Base URL `https://api.openlayer.com/v1` (or your
...
MCP connected? → use MCP. Else writing app code that already imports the SDK? → SDK. Else a one-off
shell query with no SDK installed? → SDK install is still usually cleaner than raw REST; use REST only
when you can't add a dependency.

## Common Mistakes

| Mistake | Problem | Fix |
| ------- | ------- | --- |
| Reaching for the CLI to read data | CLI is push/workflow, not a REST client | Use MCP / SDK / REST |
| Guessing SDK method names | Wrong call, runtime error | Confirm from the SDK docs/reference |
| Raw REST when SDK/MCP is available | More brittle, no types | Prefer the higher tier |
